...
function addVisibilityListeners() {
  $( '#vdc_resource_visibility_restricted' ).click( function() {
    usePrivateRequirements();
  });

  $( '#vdc_resource_visibility_open' ).click( function() {
    usePublicRequirements();
  });

  // TODO: I'm assuming that "authenticated" is not public and is essentially like private with
  //       respect to optional./required fields. Make sure this is a correct assumption.
  ...
  $( '#vdc_resource_visibility_lease' ).click( function() {
    if ( $( '#vdc_resource_visibility_during_lease' ).val() === 'open' ) {
      usePublicRequirements();
    } else {
      usePrivateRequirements();
    }
