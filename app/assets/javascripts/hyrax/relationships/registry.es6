...
export default class Registry {
  ...
  // dispatches event when tbody is touched in order to check if Collections exist in required_project  
  tableTouched(resource) {
    let event = new Event('resource-added', { 'bubbles': false, cancelable: false })
    event.resource = resource 
    event.table = this.element
